...
// All LLM traffic is routed through Mesh API (OpenAI-compatible). Model IDs
// keep their provider prefix and are passed to Mesh verbatim.
...
export async function POST(request: NextRequest) {
  try {
    const { prompt, manifest, model = 'openai/gpt-5.4' } = await request.json();
    ...
    // Mesh routes by the provider-prefixed model ID verbatim.
    // Force chat.completions compatibility — the default responses API isn't
    // supported by non-OpenAI models routed through Mesh.
    const aiModel = mesh.chat(model);
