...
// Mesh API — OpenAI-compatible, single client for every provider-prefixed model.
...
`;
        ...
        // Capability gates preserved from upstream — still useful for branching
        // on reasoning-model flags etc. — but the transport is a single Mesh client.
        const isAnthropic = model.startsWith('anthropic/');
        const isGoogle = model.startsWith('google/');
        const isOpenAI = model.startsWith('openai/');
        const isKimiGroq = model === 'moonshotai/kimi-k2-instruct-0905';
        const actualModel = model; // Mesh takes provider-prefixed IDs verbatim.

        console.log(`[generate-ai-code-stream] Routing via Mesh API, model: ${actualModel}`);

        // Make streaming API call with appropriate provider
        const streamOptions: any = {
          model: mesh.chat(actualModel),
          messages: [
            { 
              role: 'system', 
          ...
          try {
            ...
            if (retryCount < maxRetries && isRetryableError) {
              ...
              if (isGroqServiceError && retryCount === maxRetries) {
                console.log('[generate-ai-code-stream] Fallback to GPT-5.4 via Mesh');
                streamOptions.model = mesh.chat('openai/gpt-5.4');
              }
            } else {
...
Provide the complete file content without any truncation. Include all necessary imports, complete all functions, and close all tags properly.`;
                ...
                // Mesh takes the provider-prefixed model ID verbatim.
                const completionResult = await streamText({
                  model: mesh.chat(model),
                  messages: [
                    { 
                      role: 'system', 
